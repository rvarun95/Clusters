...
with tf.device('/cpu:0'):
    ...
    seed = 7
    np.random.seed(seed)
    model_CV = KerasClassifier(build_fn=create_model, epochs = epochs, verbose=0)
    ...
    grid = RandomizedSearchCV(estimator=model_CV, param_distributions=param_grid, scoring = f1, cv=2,
                              n_iter=40, verbose=5) #, random_state = np.random.seed(seed)) #, pre_dispatch=3, n_jobs=3)
...
randon_search_conv_model.fit(x_train_input, y_train, epochs=40, verbose=0)
